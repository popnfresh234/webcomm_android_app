...
function writeStringFile(plistStringJsonObj, lang, fileName) {
    ...
    switch (lang) {
        ...
        case "zh-Hant-TW":
        case "zh-TW":
            baseLProjPath = getTargetIosDir() + "/Resources/Base.lproj";
            break;
        default:
            lProjPath = null;
            break;
    ...
    fs.ensureDir(lProjPath, function (err) {
        if (!err) {
            ...
            fs.open(lProjPath + "/" + fileName, 'a', function(err, fd) {
                if(err) throw err;
    ...
    if(baseLProjPath){
        fs.ensureDir(baseLProjPath, function (err) {
            if (!err) {
                var stringToWrite = jsonToDotStrings(plistStringJsonObj);
                var buffer = iconv.encode(stringToWrite, 'utf8');
    
                fs.open(baseLProjPath + "/" + fileName, 'a', function(err, fd) {
                    if(err) throw err;
                    fs.writeFileSync(fd, buffer);
                    // fs.appendFile(fd,buffer);
                });
...
module.exports = function(context) {
    ...
    var localizableStringsPaths = ["Base.lproj/Localizable.strings"];
    var infoPlistPaths = ["Base.lproj/InfoPlist.strings"];
